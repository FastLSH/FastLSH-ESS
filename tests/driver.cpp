...
int main (int argc, char **argv){
    ...
    args theArgs;
    //read in files
    if(is_file_exist("./FastLSH_Arguments")){
        std::cout<<"FastLSH_Arguments FOUND, would you like use the parameters in the file? (Y/N)\n";
        std::cin>>input;
        if (input=="Y"){
            std::vector<std::string> argus=  argumentReader("./FastLSH_Arguments");
        ...
        std::cout<<"FastLSH_Arguments NOT FOUND, would you like to input the parameters by yourself? (Y/N)\n";
        std::cin>>input;
        if(input=="Y"){
            theArgs = readArguFromConsole();
        }
